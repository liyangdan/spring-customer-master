...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		// 正常的优先级是PriorityOrdered-> Ordered->none, 但是因为加入beanfactory的时机不同会稍微有点调整
		// Invoke BeanDefinitionRegistryPostProcessors first, if any.
		// ps:Spring中默认的优先级排序PriorityOrdered-> Ordered->none
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			//最重要。注意这里是方法调用
			//执行所有BeanDefinitionRegistryPostProcessor（目前spring内部只有一个类ConfigurationClassPostProcessor）
			...
			//先执行BeanDefinitionRegistryPostProcessor 的 postProcessBeanFactory方法
			invokeBeanFactoryPostProcessors(registryProcessors, beanFactory);
			// 执行BeanFactoryPostProcessor 的postProcessBeanFactory 方法
